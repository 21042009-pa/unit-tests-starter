  ...
  describe('POST /pedidos', () => {
    test('retorna 201 e o pedido criado com o total calculado corretamente', async () => {
      const novoPedido = {
        ...
        itens: [
          ...
          { nome: 'Refrigerante', precoUnitario: 4, quantidade: 2 },
        ],
      };
      // total esperado: 7*3 + 4*2 = 21 + 8 = 29

      const res = await request(app).post('/pedidos').send(novoPedido);

      expect(res.status).toBe(201);
      expect(res.body).toHaveProperty('id');
      expect(res.body).toHaveProperty('cliente', 'Bruno Lima');
      expect(res.body).toHaveProperty('total', 29);
    ...
    test('retorna 400 quando o cliente esta faltando', async () => {
      const res = await request(app)
        .post('/pedidos')
        .send({ itens: [{ nome: 'Coxinha', precoUnitario: 5, quantidade: 1 }] });
    ...
    test('retorna 400 quando a lista de itens esta vazia', async () => {
      const res = await request(app)
        .post('/pedidos')
        .send({ cliente: 'Carlos Melo', itens: [] });
